...
def forward_model(init_water):
    ...
    albedoC = 0.7#Planet_inputs.albedoC    #cold state albedo, 
    albedoH = 0.2#Planet_inputs.albedoH    #hot state albedo, not used currently
    '''   
    ...
    Init_solid_H2O = 5e19 #1e20#Init_conditions.Init_solid_H2O - initial water in the mantle (kg)
    ...
    #Init_solid_CO2= 1e21#Init_conditions.Init_solid_CO2  - initial CO2 in the interior (kg)

    Total_Carbon = 2e20 #BSE comp is 10^21

    # randomize the partitioning fraction between mantle and atmosphere
    Mantle_fraction = 0.8

    # calculate solid and fluid CO2 amounts
    Init_solid_CO2 = Mantle_fraction * Total_Carbon  # CO2 in the mantle
    ...
    MFrac_hydrated = 1e-2#3e-2#MC_inputs.interiorb, 1%
    ...
    def system_of_equations(t0,y):
        ...
        n_out=0.4#np.random.uniform(0.0,0.73) # Exponent determing relationship between internal heat flow and outgassing (see equation S8)
        # Calculate internal heatflow, outgassing, and spreading rate using equation 19
        ...
        dy1_dt = - escape  - total_water_loss_surf + F_H2O*0.018   #Change in surface water with time
        ...
        #y[4] = prec_plim
        #comment out y[4] when using MAC_LH_wx_flux, otherwise uncomment for WHAK and MAC (no LH)
    ...
    method='RK23', 
    t_eval=np.linspace(Start_time * 365 * 24 * 60 * 60, Max_time * 365 * 24 * 60 * 60, 5000), 
    ...
    total_time = total_time/(365*24*60*60)-Start_time ## set start time to zero
        ...
        #print((total_time), "Ocean fraction:", total_y[25], "Mantle Return:", total_y[27], "water_c:", total_y[4], "water_frac:", total_y[18])
    ...
    if plot_new_switch == "y":
        ...
        #fco2
        plt.subplot(2, 2, 4)
        plt.semilogy(total_time, total_y[23] / 1e5, color="cornflowerblue")
