...
def forward_model(T_dehyd):
    ...
    Init_fluid_H2O = 1.4e18##Earth initial water at the surface (kg)
    ...
    ### crustal hydration constant

    MFrac_hydrated = 0.005     # 3 wt% H2O in fully hydrated crust (serpentinized endmember)
    rho_crust      = 2900.0   # kg/m^3
    k_th           = 2.5      # W/m/K
    q0_modern      = 0.08     # W/m^2
    n_q            = 0.73
    #T_dehyd        = 720.0 #973.0    # K
    Pmax_hyd       = 2.0e9 #2.0e9    # Pa

    # Reference thickness used only to compute the fraction of *new* crust that can hydrate
    H_fresh        = 10000.0  # m ( pick 10–30 km; not an ocean limit)
    ...
    def system_of_equations(t0,y):
        ...
        Melt_volume_past = 40*1000.0**3/(365*24*60*60) #40
        ...
        ### for failed runs 
        total_water_gain_interior = 0.0  # default

        # Geotherm-limited hydration depth at current time 
        q_surface = q0_modern * (Q**n_q)                 # W/m^2
        G_grad    = max(q_surface / k_th, 1e-12)         # K/m
        #z_T       = max(0.0, (T_dehyd - float(y[8])) / G_grad)      # m, varying temperature
        z_T       = max(0.0, (T_dehyd - 525.0) / G_grad)      # m

        z_P       = Pmax_hyd / (rho_crust * g)                      # m
        z_hyd     = max(0.0, min(z_T, z_P))                          # m 
        y[17]      = z_hyd  # keep for plotting "Crustal Hydration Depth" (m)

        # fraction of newly produced crust  
        f_hyd = 1.0 if H_fresh <= 0 else min(1.0, z_hyd / H_fresh)

        # planet area and accessible fraction (using Ocean_fraction) 
        A_planet  = 4.0 * np.pi * rp**2

        # calculate the maximum hydration of the crust
        M_rock_hyd_zone = A_planet * rho_crust * z_hyd  # kg of rock
        Mcap_crust_inst = M_rock_hyd_zone * MFrac_hydrated         # kg H2O


        #### second calculation at 290K surface
        # Geotherm-limited hydration depth at current time 
        z_T2       = max(0.0, (T_dehyd - 290.0) / G_grad)      # m
        z_hyd2     = max(0.0, min(z_T2, z_P))                          # m 
        y[19]      = z_hyd  # keep for plotting "Crustal Hydration Depth" (m)

        M_rock_hyd_zone2 = A_planet * rho_crust * z_hyd2  # kg of rock
        Mcap_crust_inst2 = M_rock_hyd_zone2 * MFrac_hydrated         # kg H2O

        #Save some useful outputs:
        y[27] = total_water_gain_interior 

        y[4] =  Mcap_crust_inst 
        y[5] = Mcap_crust_inst2 
        ...
        ###########################################################################################
        dy0_dt = + total_water_gain_interior
        dy1_dt = - escape 
    ...
    try:
        ...
        max_step=1e5 * 365 * 24 * 60 * 60,  # Maximum step size, originally 1e5
        rtol = 1e-2
        ...
        print(f"  Error: {e}")
        print("  Inputs that caused failure:")
        #print(f"    MFrac_hydrated = {MFrac_hydrated}")
        #print(f"    n_q           = {n_q}")
        #print(f"    T_dehyd       = {T_dehyd}")
        return None, None
